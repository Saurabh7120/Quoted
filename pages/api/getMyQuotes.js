...
export default async function handler(req,res) {
    try {
        ...
        const collectionRef = query(collection(db, "quotes"), where("_user.uid", "==", uid));
        const quotesSnap = await getDocs(collectionRef);

        quotesSnap.docs.map( i => {
            const data = i.data()
            const {displayName, uid, profilePhoto} = user
            result.push({...data, _user: {displayName, uid, profilePhoto},created_on: data.created_on.toDate()});
        });
