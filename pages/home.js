import { Box, CircularProgress, Text, VStack, useDisclosure } from '@chakra-ui/react';
...
const Home = () => {
    ...
    const like = async requestId => {
        try {
            ...
            setQuotes(prev => {
                return prev.map(i => i.id === requestId ? {...i, likes: data.likes} : i)
            })
        } catch (error) {
            console.log(error)
        }
    ...
    return (
        <Layout>
           ...
           <AddQuoteModal
            isOpen={isOpen}
            onClose={onClose}
            addQuote={q => setQuotes(prev =>{ return [q,...prev]})}
           />
        </Layout>
    );
};
