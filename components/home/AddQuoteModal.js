...
const AddQuoteModal = ({isOpen,onClose, addQuote, edit, q, updateQuote}) => {
    ...
    function EditableControls() {
        ...
        return <Flex justifyContent='center' hidden={ref?.current?.value?.trim()?.length > 0 || isEditing || q}>
    ...
    return (
        ...
        <ModalContent 
            ...
            // bg={'brand.300'}
        >
          <ModalHeader>{edit ? "Edit quote" : "Quote something"}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Flex gap={'10px'}>
                <Avatar src={user?.profilePhoto} name={user?.displayName}/>
                <Box w={'100%'}>
                    <Box 
                        w={'100%'} 
                        borderRadius={'12px'} 
                        ...
                        mb={1}
                    >
                        <Text lineHeight={'0ch'}  fontSize={'4xl'}>"</Text>
                        <Editable mb={2} textAlign={'center'} defaultValue={q?.quote}>
                            <EditableControls/>
                            <EditablePreview/>
                            <EditableTextarea 
                                w={'100%'} 
                        ...
                        <Text  lineHeight={'0ch'} textAlign={'right'}  fontSize={'4xl'}>"</Text>
                    </Box>
                    {error && <Text fontWeight={"thin"} as={'span'} color={'red'}>Quote can't be empty!</Text>}
                </Box>
